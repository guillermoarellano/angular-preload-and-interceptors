...
export const httpInterceptorProviders = [
  ...
  // Log headers: Must come after the headers are stuffed, so we can log those headers.
  { provide: HTTP_INTERCEPTORS, useClass: LogHeadersInterceptor, multi: true },
  // Busy: Now we're ready to make an HTTP request, so we turn n the busy indicator. It will turn off after the request.
